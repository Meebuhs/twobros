...
class Deserializer {
  static List<User> deserializeUsersFromReference(
      List<DocumentReference> references, List<User> users) {
    return users
        .where((item) =>
            references.any((reference) => reference.documentID == item.uid))
        .toList();
  }
  ...
  static User deserializeUser(DocumentSnapshot document) {
    return User(document['uid'], document['displayName']);
  }

  static List<Chatroom> deserializeChatrooms(
      List<DocumentSnapshot> chatrooms, List<User> users) {
    return chatrooms
        .map((document) => deserializeChatroom(document, users))
        .toList();
  }

  static Chatroom deserializeChatroom(
      DocumentSnapshot document, List<User> users) {
    ...
    return Chatroom(
        deserializeUsersFromReference(participantReferences, users).toList(),
        List<Message>());
  }
  ...
  static Message deserializeMessage(
      Map<String, dynamic> document, List<User> users) {
    DocumentReference authorReference = document['author'];
    User author =
        users.firstWhere((user) => user.uid == authorReference.documentID);
